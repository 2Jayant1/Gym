...
async function seedIfEmpty(models) {
  ...
  const plans = await MembershipPlan.insertMany([
    ...
    {
      name: 'Premium',
      description: 'Unlimited access, personal trainer, and all classes.',
      monthlyFee: 79.99,
      durationDays: 30,
  ...
  const memberUser = await User.create({
    ...
    name: 'Jane Smith',
    phone: '555-0300',
    membershipPlan: standardPlan._id,
    heightCm: 168,
    weightKg: 65,
    fitnessGoals: 'Improve general fitness and flexibility',
    gender: 'female',
  ...
  for (const m of memberNames) {
    const u = await User.create({
      ...
      role: 'member',
      name: m.name,
      phone: `555-0${300 + extraMembers.length + 1}`,
      membershipPlan: rand([basicPlan._id, standardPlan._id, premiumPlan._id]),
      heightCm: m.height,
      weightKg: m.weight,
      gender: m.gender,
      fitnessGoals: m.goal,
      dateOfBirth: new Date(`199${Math.floor(Math.random() * 5) + 2}-0${Math.floor(Math.random() * 9) + 1}-15`),
    });
    extraMembers.push(u);
  }
  ...
  const yogaClass = await FitnessClass.create({
    name: 'Morning Yoga',
    description: 'Mobility and recovery class for all levels.',
  ...
  const strengthClass = await FitnessClass.create({
    name: 'Power Lift',
    description: 'Strength-focused group class with barbells.',
    trainer: trainer._id,
    durationMinutes: 50,
    difficultyLevel: 'advanced',
    category: 'strength',
  ...
  const pilatesClass = await FitnessClass.create({
    name: 'Core Pilates',
    description: 'Mat Pilates focusing on core strength and stability.',
    trainer: trainer2._id,
    durationMinutes: 55,
    difficultyLevel: 'beginner',
    category: 'yoga',
  ...
  for (let i = -3; i < 7; i++) {
    ...
    const s1 = await ClassSchedule.create({
    ...
    const s2 = await ClassSchedule.create({
      fitnessClass: hiitClass._id,
      classDate: d,
      startTime: '09:00',
      endTime: '09:45',
      room: 'Gym Floor',
    });
    const s3 = await ClassSchedule.create({
      fitnessClass: strengthClass._id,
      classDate: d,
      startTime: '17:00',
      endTime: '17:50',
      room: 'Weight Room',
    ...
    if (i % 2 === 0) {
      const s4 = await ClassSchedule.create({
        fitnessClass: pilatesClass._id,
        classDate: d,
        startTime: '11:00',
        endTime: '11:55',
        room: 'Studio B',
  ...
  const plan1 = await WorkoutPlan.create({
    ...
    description: '3-day full-body strength program',
    goal: 'general_fitness',
    difficultyLevel: 'beginner',
  ...
  for (let i = 0; i < 15; i++) {
    ...
    await models.TrainingSession.create({
      ...
      endTime: `${String(startH).padStart(2, '0')}:45`,
      durationMinutes: 45,
      sessionType: rand(['personal', 'assessment', 'group']),
      status: dayOffset < -2 ? 'completed' : 'scheduled',
      notes: 'Focus: technique + consistency.',
      cost: 30,
      rating: dayOffset < -2 ? 3 + Math.floor(Math.random() * 3) : undefined,
      feedback:
        dayOffset < -2 ? rand(['Great session!', 'Really helped with form.', 'Tough but productive.']) : undefined,
    });
  }
  ...
  for (const member of allMembers.slice(0, 6)) {
    for (let w = 0; w < 8; w++) {
      ...
      await MemberProgress.create({
        ...
        recordDate: d,
        weightKg: weight,
        bodyFatPercentage: +(18 + Math.random() * 8).toFixed(1),
        muscleMassKg: +(25 + Math.random() * 8).toFixed(1),
        chestCm: +(88 + Math.random() * 10).toFixed(1),
        waistCm: +(72 + Math.random() * 10).toFixed(1),
        hipsCm: +(92 + Math.random() * 8).toFixed(1),
        bmi,
      }).catch(() => null);
    }
  }
  ...
  const eq1 = await Equipment.create({
    ...
    brand: 'ProRun',
    model: 'XR-200',
    serialNumber: 'SN-TR-0001',
    purchaseDate: addDays(now, -400),
    purchasePrice: 2500,
    status: 'active',
    lastMaintenanceDate: addDays(now, -30),
    nextMaintenanceDate: addDays(now, 60),
  ...
  await Equipment.create({
    ...
    brand: 'IronForge',
    model: 'SM-500',
    serialNumber: 'SN-SM-0001',
    purchaseDate: addDays(now, -300),
    purchasePrice: 4500,
    status: 'active',
  ...
  await Equipment.create({
    ...
    brand: 'AquaRow',
    model: 'AR-100',
    serialNumber: 'SN-RM-0001',
    purchaseDate: addDays(now, -200),
    purchasePrice: 1800,
    status: 'maintenance',
    location: 'Cardio Zone',
  ...
  await EquipmentMaintenance.create({
    equipment: eq1._id,
    maintenanceDate: addDays(now, -30),
  ...
  const ticketTitles = [
    ...
    'Broken treadmill belt',
    'Question about class schedule',
    'Refund request',
  ...
  for (let i = 0; i < Math.min(4, tickets.length); i++) {
    ...
    if (i < 2) {
      await TicketReply.create({
        ticket: tickets[i]._id,
        user: adminUser._id,
        message: 'Internal note: escalated to management.',
        isInternal: true,
      });
    }
  }
